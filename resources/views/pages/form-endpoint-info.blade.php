...
@section('title', $verified ? 'Endpoint Active' : 'Activate Your Form Endpoint')
        ...
        @if($verified)
            ...
            <div style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 12px; padding: 1.5rem; margin-bottom: 1.5rem;">
                ...
                <div style="background: var(--bg-primary); border: 1px solid var(--border-color); border-radius: 6px; padding: 0.875rem 1rem; font-family: var(--font-mono); font-size: 0.85rem; color: var(--accent); word-break: break-all;">
                    {{ config('app.url') }}/f/{{ $email }}
                </div>
                <p style="color: var(--text-muted); font-size: 0.8rem; margin-top: 0.75rem; margin-bottom: 0;">
                    Use this as the <code style="color: var(--accent);">action</code> attribute in any HTML form. Submissions will be sent to <strong style="color: var(--text-primary);">{{ $email }}</strong>.
                </p>
            </div>
            ...
            <div style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 12px; padding: 1.5rem; margin-bottom: 1.5rem;">
                ...
                <pre style="background: var(--bg-primary); border: 1px solid var(--border-color); border-radius: 6px; padding: 1rem; font-family: var(--font-mono); font-size: 0.8rem; color: var(--text-secondary); overflow-x: auto; margin: 0; white-space: pre-wrap;">&lt;form action="{{ config('app.url') }}/f/{{ $email }}" method="POST"&gt;
...
&lt;/form&gt;</pre>
        ...
        @else
            ...
            <div style="text-align:center; margin-bottom: 2rem;">
                ...
                <p style="color: var(--text-secondary); font-size: 0.95rem;">
                    A verification email has been sent to <strong style="color: var(--text-primary);">{{ $email }}</strong>.
            ...
            <div style="background: rgba(255,170,0,0.08); border: 1px solid rgba(255,170,0,0.2); border-radius: 8px; padding: 1rem 1.25rem; margin-bottom: 1.5rem;">
                <p style="color: var(--warning); font-size: 0.875rem; margin: 0; line-height: 1.6;">
                    ...
                    <code style="font-family: var(--font-mono);">{{ config('app.url') }}/f/{{ $email }}</code>
