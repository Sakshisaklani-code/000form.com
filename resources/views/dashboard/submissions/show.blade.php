                ...
                @if(!str_starts_with($key, '_'))
                    <div>
                        ...
                        <div style="padding: 0.75rem 1rem; background: var(--bg-tertiary); border-radius: 6px; word-break: break-word;">
                            ...
                            @if(is_array($value))
                                {{-- Check if it's the 'uploads' key with multiple files --}}
                                @if($key === 'uploads' && isset($value[0]) && is_array($value[0]) && isset($value[0]['name']))
                                ...
                                {{-- Single file upload (old format or 'upload' key) --}}
                                @elseif(isset($value['name']) && isset($value['size']) && isset($value['path']))
                                    @php
                ...
                @if($submission->metadata && isset($submission->metadata['has_attachment']) && $submission->metadata['has_attachment'])
                    <div style="display: flex; justify-content: space-between; padding: 0.5rem 0; border-bottom: 1px solid var(--border-color);">
                        ...
                        <span style="color: var(--success);">
                            {{ $submission->metadata['attachment_count'] ?? 1 }} file(s)
                        </span>
                    </div>
