...
<div class="dash-header au d1">
    <div class="dash-title-wrap">
        <div class="dash-eyebrow">
            <span class="proj-color-dot" style="background:{{ $project->color }};box-shadow:0 0 8px {{ $project->color }}55;"></span>
             project
        </div>
        <h1 class="dash-title">{{ $project->name }}</h1>
        @if($project->description)
            <p class="proj-desc-sub">{{ $project->description }}</p>
        @endif
    </div>
    <div class="header-actions">
        <a href="{{ route('dashboard.forms.create', ['project_id' => $project->id]) }}" class="btn-new">
...
@if($forms->count() > 0)
    ...
    <div class="forms-card au" style="animation-delay:.38s;">
        <div class="color-strip" style="background:{{ $project->color }};"></div>
                    ...
                    <tr data-search="{{ strtolower($form->name . ' ' . $form->slug . ' ' . $form->status) }}">
                        <td>
                            <a href="{{ route('dashboard.forms.show', $form->id) }}"
                               class="t-link" data-label="{{ $form->name }}">
                                {{ $form->name }}
                            </a>
                            @if($form->unread_count > 0)
                                <span class="badge b-g" style="margin-left:.5rem;">
                                    <span class="bd"></span>{{ $form->unread_count }} new
                                </span>
                            @endif
                        </td>
                        ...
                        <td class="tr">
                            <a href="{{ route('dashboard.forms.show', $form->id) }}" class="btn-tbl">View</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
