...
<style>
    ...
    .results-info { font-size: 0.78rem; color: var(--text-muted); padding: 0.5rem 0 0.1rem; }
    ...
    .pagination-link:not(.disabled):not(.active):hover {
        background: var(--bg-tertiary); color: var(--text); border-color: var(--border-color);
    }
    ...
    .validation-card:hover { box-shadow: 0 2px 8px rgba(0,0,0,0.08); }
    .validation-card-icon {
        width: 32px; height: 32px;
        border-radius: 6px;
        background: rgba(65,66,101,0.1);
        display: flex; align-items: center; justify-content: center;
        flex-shrink: 0;
    }
    .validation-card-info { flex: 1; margin-left: 0.75rem; }
    .validation-card-name { font-size: 0.875rem; font-weight: 600; color: var(--text); margin: 0 0 0.2rem; }
    .validation-card-meta { font-size: 0.75rem; color: var(--text-muted); margin: 0; }
    ...
    .validation-card-badge.required { background: rgba(239,68,68,0.12); color: #ef4444; }
    .validation-card-badge.optional { background: rgba(148,163,184,0.15); color: var(--text-muted); }
    ...
    .validation-card-btn:hover { background: var(--bg-tertiary); color: var(--text); }
    .validation-card-btn.delete:hover { background: rgba(239,68,68,0.1); color: #ef4444; }
    ...
    .form-label span { font-size: 0.72rem; color: var(--text-muted); font-weight: 400; }
    .form-control {
    ...
    .form-control:focus { border-color: #414265; }
...
<div class="stats-grid mb-3" style="grid-template-columns: repeat(4, 1fr);">
    ...
    <div class="card stat-card">
        ...
        <div class="stat-value" style="color:#6366f1;">{{ number_format($archiveCount) }}</div>
    </div>
    <div class="card stat-card">
...
<nav class="page-tabs-bar">
    ...
    {{-- ── NEW: Workflow Tab ── --}}
    <a href="{{ request()->fullUrlWithQuery(['panel' => 'workflow']) }}"
       class="page-tab {{ $panel === 'workflow' ? 'active' : '' }}">
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        @if(isset($validations) && $validations->count() > 0)
            <span class="tab-pill" style="background:rgba(65,66,101,0.15);color:#414265;">
                {{ $validations->count() }}
            </span>
        @endif
    </a>
...
{{-- ══════════════════════════════
     PANEL: SUBMISSIONS
══════════════════════════════ --}}
@php $tab = request('tab', 'valid'); @endphp
    ...
    <div class="submissions-section">
        <div class="submissions-section-header">
            ...
            <div class="submission-tabs-bar" style="padding-bottom:0;">
                <div class="submission-tabs">
                    ...
                    <a href="{{ request()->fullUrlWithQuery(['panel' => 'submissions', 'tab' => 'archive', 'search' => $search, 'page' => 1]) }}"
                       class="submission-tab {{ $tab === 'archive' ? 'active' : '' }}">
                        ...
                        Archive
                        @if(isset($archiveCount) && $archiveCount > 0)
                            <span class="tab-pill archive">{{ number_format($archiveCount) }}</span>
                        @endif
                    </a>
                </div>
            ...
            @if($search && $tab !== 'archive')
                <p class="results-info">
                    @if($submissions->total() > 0)
                        <strong>{{ $submissions->total() }}</strong> result{{ $submissions->total() !== 1 ? 's' : '' }}
        ...
        @if($tab === 'archive')
            <div class="archive-toggle-banner">
                ...
                <form method="POST" action="{{ route('dashboard.forms.toggle-archive', $form->id) }}"
                      id="archiveToggleForm" style="margin:0;">
                    @csrf
                    @method('PATCH')
                    <label class="toggle-switch">
                        <input type="checkbox" id="archiveToggle" name="archive_when_paused" value="1"
                               {{ ($form->archive_when_paused ?? true) ? 'checked' : '' }}
                               onchange="document.getElementById('archiveToggleForm').submit()">
                        <span class="toggle-track"></span>
                        <span class="toggle-label" id="toggleLabel">
                            {{ ($form->archive_when_paused ?? true) ? 'On' : 'Off' }}
                        </span>
                    </label>
                </form>
            </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        @foreach($submissions as $submission)
                            <tr class="{{ $submission->is_spam ? 'spam-row' : ($submission->is_archived ? 'archive-row' : '') }}">
                                <td>
                                    <a href="{{ route('dashboard.submissions.show', [$form->id, $submission->id]) }}"
                                       class="table-link">
                                        {{ Str::limit($submission->summary, 60) }}
                                    </a>
                                    @if(!$submission->is_read && !$submission->is_spam && !$submission->is_archived)
                                        <span class="badge badge-success" style="margin-left:0.5rem;">New</span>
                                    @endif
                                    @if($submission->is_spam)
                                        <span style="margin-left:0.5rem;background:rgba(245,158,11,0.15);color:#f59e0b;font-size:0.68rem;padding:0.15rem 0.45rem;border-radius:999px;display:inline;">Spam</span>
                                    @endif
                                    @if($submission->is_archived)
                                        <span style="margin-left:0.5rem;background:rgba(99,102,241,0.15);color:#6366f1;font-size:0.68rem;padding:0.15rem 0.45rem;border-radius:999px;display:inline;">Archived</span>
                                        @if(isset($submission->metadata['archived_reason']) && $submission->metadata['archived_reason'] === 'form_paused')
                                            <span style="font-size:0.72rem;color:var(--text-muted);margin-left:0.35rem;">· received while paused</span>
                                        @endif
                                    @endif
                                    @if($submission->spam_reason)
                                        <span style="font-size:0.72rem;color:var(--text-muted);margin-left:0.35rem;">· {{ $submission->spam_reason }}</span>
                                    @endif
                                </td>
            ...
            @if($submissions->hasPages() || $submissions->total() > 0)
                <div class="pagination-footer">
                    <div class="pagination-info">
                        ...
                        <span>
                            Showing <strong>{{ $submissions->firstItem() }}</strong> to
                            <strong>{{ $submissions->lastItem() }}</strong> of
                            <strong>{{ $submissions->total() }}</strong> results
                        </span>
                    </div>
            ...
            <div class="empty-state" style="padding:3rem 1.5rem;">
                ...
                @elseif($search)
                    ...
                    <p class="empty-description">
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['search' => '', 'page' => 1]) }}" style="color:#414265;">Clear search</a>
                    </p>
                @elseif($tab === 'spam')
                    <svg class="empty-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
...
<div class="page-panel {{ $panel === 'statistics' ? 'active' : '' }}">
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin-bottom:1.25rem;">
        <div class="card" style="padding:1rem 1.25rem;">
            ...
            <p style="margin:0;font-size:1.6rem;font-weight:700;color:#22c55e;">{{ number_format($validCount) }}</p>
        </div>
        <div class="card" style="padding:1rem 1.25rem;">
            ...
            <p style="margin:0;font-size:1.6rem;font-weight:700;color:#f59e0b;">{{ number_format($spamCount) }}</p>
        </div>
        <div class="card" style="padding:1rem 1.25rem;">
            ...
            <p style="margin:0;font-size:1.6rem;font-weight:700;color:#6366f1;">{{ number_format($archiveCount) }}</p>
        </div>
    </div>
    <div class="charts-grid" style="display:grid;grid-template-columns:2fr 1fr;gap:1.25rem;align-items:start;">
        ...
        <div style="padding:1.25rem;">
            ...
            <div id="code-html" class="code-block active">
                ...
                <div class="code-content">
                    <pre id="html-pre"><span class="tag">&lt;form</span> <span class="attr">action</span>=<span class="string">"{{ $form->endpoint_url }}"</span> <span class="attr">method</span>=<span class="string">"POST"</span><span class="tag">&gt;</span>
...
{{-- ══════════════════════════════
     PANEL: WORKFLOW
══════════════════════════════ --}}
<div class="page-panel {{ $panel === 'workflow' ? 'active' : '' }}">

    {{-- Info banner --}}
    <div style="display:flex;align-items:flex-start;gap:0.75rem;padding:0.9rem 1.1rem;background:rgba(65,66,101,0.06);border:1px solid rgba(65,66,101,0.15);border-radius:8px;margin-bottom:1.25rem;">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#414265" stroke-width="2" style="flex-shrink:0;margin-top:1px;">
            <circle cx="12" cy="12" r="10"/>
            <line x1="12" y1="8" x2="12" y2="12"/>
            <line x1="12" y1="16" x2="12.01" y2="16"/>
        </svg>
        <p style="margin:0;font-size:0.82rem;color:var(--text-muted);line-height:1.5;">
            <strong style="color:var(--text);">How Workflow works:</strong>
    ...
    <div class="workflow-section">
        <div class="workflow-section-header">
            <div>
                ...
                <p class="workflow-section-desc">
                    Backend validations check to ensure the data is correct before processing a submission.
                </p>
            </div>
            <button class="btn btn-secondary btn-sm" onclick="openValidationModal()">
                + Add New
            </button>
        </div>
                        ...
                        <div style="display:flex;align-items:flex-start;gap:0.75rem;flex:1;">
                            <div class="validation-card-icon">
                                @if($v->field_type === 'email')
                                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#414265" stroke-width="2">
                                        <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
                                        <polyline points="22,6 12,13 2,6"/>
                                    </svg>
                                @elseif($v->field_type === 'number')
                                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#414265" stroke-width="2">
                            ...
                            <div class="validation-card-info">
                                <p class="validation-card-name">{{ $v->field_name }}</p>
                                <p class="validation-card-meta">
                                    {{ ucfirst($v->field_type) }}
                                    @if($v->min_length || $v->max_length)
                                        ·
                                        @if($v->min_length) min {{ $v->min_length }} @endif
                                        @if($v->min_length && $v->max_length) / @endif
                                        @if($v->max_length) max {{ $v->max_length }} @endif
                                        chars
                                    @endif
                                </p>
                                <span class="validation-card-badge {{ $v->is_required ? 'required' : 'optional' }}">
        ...
        @else
            <div class="empty-state" style="padding:2.5rem 1.5rem;">
                ...
                <button class="btn btn-secondary btn-sm" style="margin-top:0.75rem;" onclick="openValidationModal()">
        ...
        <form id="validationForm">
            ...
            <div class="form-group">
                <label class="form-label" for="fieldName">
                    Field Name
                    <span>— Must match the form <code>name</code> attribute.</span>
                </label>
                <input type="text" id="fieldName" name="field_name" class="form-control"
                       placeholder="e.g. message, email, name" required>
            </div>
            ...
            <div class="form-row" id="lengthFields">
                <div class="form-group">
                    ...
                    <input type="number" id="minLength" name="min_length" class="form-control"
                           placeholder="e.g. 10" min="1">
                </div>
                <div class="form-group">
                    <label class="form-label" for="maxLength">Maximum Length</label>
                    <input type="number" id="maxLength" name="max_length" class="form-control"
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary btn-sm" onclick="closeValidationModal()">Cancel</button>
...
<div class="modal-overlay" id="deleteModal">
    <div class="modal-box" style="max-width:380px;text-align:center;">
        ...
        <h3 style="margin:0 0 0.5rem;font-size:1rem;font-weight:700;color:var(--text);">Delete Validation?</h3>
        <p style="margin:0 0 1.25rem;font-size:0.85rem;color:var(--text-muted);">This validation rule will be permanently removed.</p>
        <div style="display:flex;gap:0.75rem;justify-content:center;">
            <button class="btn btn-secondary btn-sm" onclick="closeDeleteModal()">Cancel</button>
            <button class="btn btn-sm" style="background:#ef4444;color:white;border:none;" id="confirmDeleteBtn">Delete</button>
        </div>
    </div>
</div>
...
<script>
    ...
    window.__rk = '{{ config('services.recaptcha.site_key') }}';
    ...
    if (document.getElementById('lineChart')) {
        ...
        new Chart(document.getElementById('barChart'), {
            ...
            data: {
                ...
                datasets: [{
                    data: [{{ $validCount }}, {{ $spamCount }}, {{ $archiveCount }}],
        ...
        if (archiveCanvas) {
            new Chart(archiveCanvas, {
                ...
                data: {
                    labels: @json($lineLabels),
                    datasets: [{
                        label: 'Archived', data: @json($archiveLineData),
    ...
    function updateModalTitle() {
        ...
        // Hide length fields for email type
    ...
    document.getElementById('validationForm').addEventListener('submit', async function(e) {
        ...
        try {
            const url = isEdit
                ? `/dashboard/forms/${FORM_ID}/validations/${id}`
                : `/dashboard/forms/${FORM_ID}/validations`;
